import { pgTable, text, timestamp, uuid, boolean, jsonb, date, time, integer, uniqueIndex } from 'drizzle-orm/pg-core';
...
export const scheduleExceptions = pgTable('schedule_exceptions', {
    ...
    memberDateUnique: uniqueIndex('member_date_unique').on(table.memberId, table.date),
}));

// ==========================================
// SOLICITAÇÕES DE DISPONIBILIDADE (LINKS PÚBLICOS)
// ==========================================

export const availabilityRequests = pgTable('availability_requests', {
    id: uuid('id').defaultRandom().primaryKey(),
    memberId: uuid('member_id').references(() => members.id, { onDelete: 'cascade' }).notNull(),
    token: text('token').notNull().unique(), // Token unico para link publico
    status: text('status', { enum: ['pending', 'completed', 'expired'] }).default('pending').notNull(),
    expiresAt: timestamp('expires_at').notNull(),
    period: text('period').notNull(), // '7_days', '15_days', '30_days'
    createdAt: timestamp('created_at').defaultNow().notNull(),
...
export const availabilityRequestsRelations = relations(availabilityRequests, ({ one }) => ({
    member: one(members, {
        fields: [availabilityRequests.memberId],
        references: [members.id],
    }),
}));
